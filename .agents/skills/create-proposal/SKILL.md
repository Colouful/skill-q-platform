...
- **使用技能**：`.agents/skills/common/design-analysis/SKILL.md`
...
- 涉及 UI 时：组件放置位置建议（依据 `.agents/rules/profiles/<profile>/04-组件规范.md`）
- 涉及接口时：接口结构建议（依据 `.agents/rules/common/05-API规范.md`）
- 接口未就绪时：标注 mock 策略
...
### 4.1 design.md 检查
- 技术方案是否遵循 `.agents/rules/common/` 与 `.agents/rules/profiles/<profile>/` 中的架构约束
- 涉及页面时，是否参考了 `.agents/rules/profiles/<profile>/06-路由规范.md`
- 涉及组件时，是否参考了 `.agents/rules/profiles/<profile>/04-组件规范.md`
- 样式方案是否使用主题变量（`.agents/rules/profiles/<profile>/09-样式规范.md`）

### 4.2 tasks.md 检查
- 涉及 UI 且有设计稿时，末尾是否包含 UI 还原验收任务（引用 `.agents/skills/common/ui-verification/SKILL.md`）
- 涉及接口时，是否包含接口封装任务（引用 `.agents/rules/common/05-API规范.md`）
- 图标/图片未定时，是否标注占位元素（`.agents/rules/common/08-通用约束.md`）
...
- 提案确认后进入执行阶段时，遵循 `.agents/rules/common/12-Superpowers执行规范.md`，按 `.agents/skills/common/execute-task/SKILL.md` 的四步循环逐条执行 `openspec/changes/<change-name>/tasks.md`，或通过 IDE/OpenSpec 的 apply 命令进入同一执行链路。
- **本步骤 4 后置检查完成、向用户交付分析摘要与检查结果时**，须遵守 `.agents/rules/common/14-审计汇报规范.md`（适用范围含「完成 create-proposal 后置检查」）。执行阶段的审计要求见 **4.5** 中与方案一、方案二的衔接说明。

### 4.5 输出模板：下一步（必选）

OpenSpec 生成提案且本步骤 4 检查完毕后，**必须**以独立小节 **「下一步」** 输出以下内容（将 `<change-name>` 替换为实际变更目录名，如 `add-simple-input`）。

并固定说明：**默认执行方案一（Superpowers + execute-task）**；方案二仅在用户明确提出 direct 类关键词时启用。
...
> 默认执行此方案。请使用 execute-task 技能，以 `openspec/changes/<change-name>/tasks.md` 为准，从第一条未勾选 `- [ ]` 开始，按 Superpowers 四步（头脑风暴 → TDD → 双重审查 → 状态更新）逐条执行；每条 task 在第一步都必须主动提问“我的思路是否正确？是否可以开始编写测试和代码？”并等待明确同意。必要时先执行 `/opsx-apply <change-name>`（Cursor 等）或 `/opsx:apply <change-name>`（Claude Code）。执行须遵守 `.agents/rules/common/12-Superpowers执行规范.md` 与 `.agents/rules/common/14-审计汇报规范.md`（第四步状态更新含审计报告）。

技能路径：`.cursor/skills/common/execute-task/SKILL.md` 或 `.agents/skills/common/execute-task/SKILL.md`。

#### 4.5.3 方案二：仅按 tasks.md 清单

适合小改动或用户明确要求「快扫清单」、不强制每步输出四步标题时，提供**可复制**提示语。仅当用户显式出现 `直接执行`、`快扫清单`、`不使用superpower`、`不使用 Superpowers`、`direct` 之一时启用：

> 请直接按 `openspec/changes/<change-name>/tasks.md` 顺序逐项实现，每完成一项将对应 `- [ ]` 改为 `- [x]`，并保证与 `design.md`、`specs/` 一致。**凡产生代码变更**，仍须按 `.agents/rules/common/14-审计汇报规范.md` 输出审计报告，不得因未走 execute-task 四步而省略。
...
create-route、create-component 等技能中「涉及 UI 还原时」可引用：`.agents/skills/common/create-proposal/SKILL.md` 中的「样式还原验证检查清单」及对应页面的 `docs/样式还原/<名称>-UI分析清单.md`。

---

## 相关规范与技能

- `.agents/rules/profiles/<profile>/03-项目结构.md` - 目录结构、Mock 数据策略
- `.agents/rules/profiles/<profile>/04-组件规范.md` - 组件放置决策
- `.agents/rules/common/05-API规范.md` - 接口封装
- `.agents/rules/profiles/<profile>/06-路由规范.md` - 路由结构
- `.agents/rules/common/08-通用约束.md` - 占位元素等
- `.agents/rules/profiles/<profile>/09-样式规范.md` - 设计稿颜色提取、主题变量
- `.agents/rules/common/12-Superpowers执行规范.md` - 执行原则
- `.agents/rules/common/14-审计汇报规范.md` - 后置检查交付与执行任务后的审计报告
- `.agents/skills/common/execute-task/SKILL.md` - Superpowers 四步循环执行
- `.agents/skills/common/design-analysis/SKILL.md` - 设计稿分析（有设计稿时使用，产出 UI 分析清单）
- `.agents/skills/common/ui-verification/SKILL.md` - UI 验收（实现后需验收时使用）
