...
describe("AssetQueryService", () => {
  ...
  it("keyword 应支持 slug / name / description 模糊搜索", () => {
    ...
    expect(query.list(new URLSearchParams("keyword=查询规划")).items[0].slug).toBe("query-planner-role");
    expect(query.list(new URLSearchParams("keyword=query-implementation")).items[0].slug).toBe(
  ...
  it("应按 kind 筛选", () => {
    ...
    const result = query.list(new URLSearchParams("kind=flow"));
  ...
  it("详情应返回版本摘要、引用和统计", () => {
    ...
    const detail = query.detail(role.asset.id);
  ...
  it("非法分页应报 INVALID_PAGINATION", () => {
    const { query } = seedQueryRepo();

    expect(() => query.list(new URLSearchParams("page=0&pageSize=20"))).toThrow("分页参数不合法");
  });
});
