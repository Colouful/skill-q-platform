...
describe("AgentProfileQueryService", () => {
  it("应支持分页、keyword、status 和 riskLevel 筛选", () => {
    ...
    expect(service.list(new URLSearchParams("page=1&pageSize=1")).pagination.total).toBe(2);
    expect(service.list(new URLSearchParams("keyword=high")).items).toHaveLength(1);
    expect(service.list(new URLSearchParams("status=published")).items[0].slug).toBe("query-high-agent");
    expect(service.list(new URLSearchParams("riskLevel=low")).items[0].slug).toBe("query-low-agent");
  });
});
