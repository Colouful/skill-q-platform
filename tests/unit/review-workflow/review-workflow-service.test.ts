...
describe("ReviewWorkflowService", () => {
  it("Asset Version draft 可提交审核、reviewing 可驳回、rejected 可重新提交审核", () => {
    const { audit, workflow, asset, version } = createAssetVersionFixture();

    expect(workflow.submitAssetVersion(asset.id, version.id, { note: "提交" }).version.status).toBe("reviewing");
    expect(workflow.rejectAssetVersion(asset.id, version.id, { reason: "内容需要调整" }).version.status).toBe("rejected");
    expect(workflow.submitAssetVersion(asset.id, version.id, { note: "重新提交" }).version.status).toBe("reviewing");
    expect(audit.list({ targetId: version.id }).items).toHaveLength(3);
  });

  it("Asset Version reviewing 可发布，published 不可提交审核", () => {
    const { workflow, asset, version } = createAssetVersionFixture();
    workflow.submitAssetVersion(asset.id, version.id, {});

    const published = workflow.publishAssetVersion(asset.id, version.id, {});

    expect(published.version.status).toBe("published");
    expect(() => workflow.submitAssetVersion(asset.id, version.id, {})).toThrow("当前状态不允许提交审核");
  });

  it("archived Asset 不允许提交审核且 published 不允许修改内容", () => {
    const { workflow, asset, version, assetService } = createAssetVersionFixture();
    assetService.archive(asset.id, { reason: "归档" });

    expect(() => workflow.submitAssetVersion(asset.id, version.id, {})).toThrow("已归档资源不允许进入审核流程");
  });

  it("Manifest Version draft 可提交审核、reviewing 可驳回、reviewing 可发布", () => {
    const { audit, workflow, manifest, version } = createManifestVersionFixture();

    expect(workflow.submitManifestVersion(manifest.id, version.id, {}).version.status).toBe("reviewing");
    expect(workflow.rejectManifestVersion(manifest.id, version.id, { reason: "缺少说明" }).version.status).toBe("rejected");
    workflow.submitManifestVersion(manifest.id, version.id, {});
    const published = workflow.publishManifestVersion(manifest.id, version.id, {});

    expect(published.version.status).toBe("published");
    expect(audit.list({ targetId: version.id }).items.length).toBeGreaterThanOrEqual(4);
  });

  it("Manifest Version 无 required asset 时发布失败", () => {
    const repo = createHubRepository();
    const manifestService = new ManifestGovernanceService(repo);
    const versionService = new ManifestVersionService(repo);
    ...
    const version = versionService.create(manifest.manifest.id, { version: "1.0.0" });
    workflow.submitManifestVersion(manifest.manifest.id, version.version.id, {});

    expect(() => workflow.publishManifestVersion(manifest.manifest.id, version.version.id, {})).toThrow(
      "Manifest 至少需要绑定一个 required asset",
    );
  });

  it("Agent Profile draft 可提交审核、reviewing 可驳回、reviewing 可发布", () => {
    const { audit, workflow, profile } = createAgentProfileFixture();

    expect(workflow.submitAgentProfile(profile.id, {}).profile.status).toBe("reviewing");
    expect(workflow.rejectAgentProfile(profile.id, { reason: "策略说明不足" }).profile.status).toBe("rejected");
    workflow.submitAgentProfile(profile.id, {});
    const published = workflow.publishAgentProfile(profile.id, {});

    expect(published.profile.status).toBe("published");
    expect(audit.list({ targetType: "agent-profile", targetId: profile.id }).items.length).toBeGreaterThanOrEqual(4);
  });

  it("Agent Profile validate 不通过时发布失败", () => {
    ...
    expect(() => workflow.publishAgentProfile(profile.id, {})).toThrow("当前状态不允许发布");
  });
  ...
  it("reason 为空时驳回失败", () => {
    ...
    expect(() => workflow.rejectAssetVersion(asset.id, version.id, { reason: "" })).toThrow("驳回原因不能为空");
  });
});
