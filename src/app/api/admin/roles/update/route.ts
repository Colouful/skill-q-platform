...
export async function POST(req: Request) {
  try {
    ...
    const existing = await prisma.roleTemplate.findUnique({
      where: { id: b.id },
      select: { id: true, slug: true, registryId: true, manifestId: true },
    });
    if (!existing) {
    ...
    const duplicateIssues = await assertUniqueRegistryFields({
      resourceType: "role",
      registryId: b.registryId === undefined ? existing.registryId : registryId,
      manifestId: b.manifestId === undefined ? existing.manifestId : manifestId,
      excludeId: b.id,
    });
