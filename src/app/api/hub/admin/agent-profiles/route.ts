...
export async function GET(req: Request) {
  try {
    const data = new AgentProfileQueryService(defaultHubRepository).list(new URL(req.url).searchParams);
...
export async function POST(req: Request) {
  try {
    ...
    const data = new AgentProfileGovernanceService(defaultHubRepository).createDraft(body);
    return hubSuccess(data);
  } catch (error) {
    return hubException(error);
  }
