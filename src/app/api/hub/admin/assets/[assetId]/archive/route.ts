...
import { AssetGovernanceService, defaultHubRepository } from "@/server/hub";
...
export async function POST(req: Request, { params }: { params: Promise<{ assetId: string }> }) {
  try {
    ...
    const data = new AssetGovernanceService(defaultHubRepository).archive(decodeURIComponent(assetId), body);
