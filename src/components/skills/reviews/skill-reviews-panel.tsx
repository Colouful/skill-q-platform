...
export function SkillReviewsPanel({
  ...
  return (
    <section className="space-y-4 border-t-4 border-[var(--pixel-border)] pt-6">
      ...
      <SkillReviewForm slug={slug} onSuccess={onReviewCreated} />
      ...
      {loading && reviews.length === 0 ? (
        ...
        <ul className="space-y-3">
          {reviews.map((r) => (
            <li
              key={r.id}
              className="border-2 border-[var(--pixel-border)] bg-[#fffef8] p-3 font-[family-name:var(--font-pixel-body)] text-sm"
            >
